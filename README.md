...
We are using an ST Link V2 serial wire debug (SWD) adapter for flashing and debugging the code together with the Rust toolchain and crates
for targetting MCUs.

## Prerequisites

You'll need Rust, Just and the usual suspects.
...
Run `just build` to compile the code. Run `just` alone to see all available commands.
...
2. While continuing to hold down `BOOT0`, press the `NRST` button once, then run `just flash` to flash the binary onto the MCU.
